...
DEBATE_SYSTEM_PROMPT = """You are simulating a lively, intelligent, and in-character team discussion in SynapseGuild.
...
Output MUST be valid JSON strictly matching this schema:
{
  "dialogues": [
    {
      "actor": "architect", // "architect" | "craftsman" | "sentinel"
      "speech": "Natural conversational dialogue in Indonesian (1-2 sentences)",
      "target_zone": "war_room" // "war_room" | "forge" | "judgment_chamber"
    },
    {
      "actor": "craftsman",
      "speech": "Natural conversational dialogue in Indonesian (1-2 sentences)",
...
def generate_party_discussion(quest_goal: str, phase: str, context: str = "") -> list:
    prompt = DEBATE_SYSTEM_PROMPT.format(quest_goal=quest_goal, phase=phase, context=context)
    messages = [
